...
export const iconsGenPlugin: MoniconPlugin<MyPluginOptions> = (options) => () => {
	...
	return {
		...
		generate: (ctx) => {
			...
			const typesString = ctx.icons.map((icon) => `"${icon.name}"`).join(" | ");

			const entriesString = entries.join(", \n\n");

			files.push({
				content: getIconFileContent({ entriesString, typesString }),
...
const getIconFileContent = (options: { entriesString: string; typesString: string }) => {
	const { entriesString, typesString } = options;

	return `// This file is automatically generated by Monicon. Do not edit this file directly.
export type MoniconIconNameType = ${typesString};
